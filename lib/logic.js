...
async function makeVote(vote_request) {
    ...
    if((election.allowed_voter_ids.indexof(vote_request.vote_id) > -1) && new_secret_voter.voted == false){
        const new_secret_voter = factory.newResource(namespace, 'SecretVoter', vote_request.vote_id)
        new_secret_voter.voted = true

        new_secret_voter.candidate = factory.newRelationship(namespace, 'Candidate', vote_request.candidate_id)

        const secretVoterRegistry = await getAssetRegistry(new_secret_voter.getFullyQualifiedType())
        await secretVoterRegistry.add(new_secret_voter)
    }
...
async function createElection(election_request) {
    ...
    newElection.manager = factory.newRelationship(electionManager.getFullyQualifiedName())
...
async function addElectionMembers(request_details) {
    ...
    for(const candidateInfo of request_details.candidates){
        ...
        candidate.election = factory.newRelationship(election.getFullyQualifiedName())
    ...
    voters = []
    for(const voter_email of request_details.voters){
        const voter = factory.newResource(namespace, 'Voter', voter_email)
        voter.election = factory.newRelationship(election.getFullyQualifiedName())
        voters.push(voter)
    }
...
async function startElection(start_election_request) {
    const electionRegistry = await getAssetRegistry(namespace + 'Election')
    const election = electionRegistry.get(start_election_request.election_id)
...
async function endElection(end_election_request) {
    const electionRegistry = await getAssetRegistry(namespace + 'Election')
    const election = electionRegistry.get(end_election_request.election_id)
    if(!election) throw new Error('Election does not exist');
